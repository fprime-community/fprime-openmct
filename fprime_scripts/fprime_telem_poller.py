...
class TelemPipeline(StandardPipeline):
    ...
    5. heli_states -> Contains dictionary of telemetry object names and their immediate value. Used to initialize values and define the telemetry states to be processed by OpenMCT server 
    6. heli_json -> Contains JSON of telemetry object names and their value. Saved in OpenMCT directory and used by OpenMCT server to define initial telemetry states.
    ...
    def set_telem_json(self):
        ...
        for hist in self.telem_hist:
            ...
            self.heli_states[hist_name] = hist.get_val()

            self.telem_data.append(hist_data)

    def write_telem_json(self, fname="openmct-tutorial/data.json"):
        self.heli_json = json.dumps(self.heli_states, indent=4)
        with open(fname, "w") as outfile:
            outfile.write(self.heli_json)

    def post_telem(self, uri="http://127.0.0.1:4052/telem"):
...
parser.add_argument('--openmct-uri', dest='openmct_uri', type=str, required=False, default="http://127.0.0.1:4052/telem", help="URI of the OpenMCT Server. The URI at which the F-Prime telemetry will be broadcasted.")
...
while True:
    ...
    #Write initial states to data.json and save it in the user-specified OpenMCT directory
    if telem_pipeline.json_writeable and write_json:
        telem_pipeline.write_telem_json(args.openmct_dir + "data.json")
        write_json = False
